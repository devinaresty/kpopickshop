  ...
  <section class="py-12 sm:py-16 lg:py-20 bg-white">
    <div class="max-w-full px-3 sm:px-4 lg:px-8">
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
        ...
        >
          <div
            class="relative bg-white rounded-lg overflow-hidden transition-transform duration-300 hover:shadow-lg border border-gray-200"
...
const addToCart = (product: Product) => {
  const cartProduct = {
    ...
    stock: product.stock,
    category: (product as any).category?.name || 'Other',
...
const reloadProducts = async () => {
  try {
    ...
    error.value = null
    const response = await apiClient.getProducts(0, 18)
